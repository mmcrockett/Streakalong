app.controller('StreaksController', [
'$scope',
'$timeout',
'Activity',
'Calories',
...
function(
  ...
  $scope.RANGE_OPTIONS      = _.constant([
    'day',
    'week',
    'month',
    'year'
  ]);
  $scope.selected_range     = $scope.RANGE_OPTIONS()[0];
  save_timeout = null;
  ...
  // NEW
/*
  $scope.stacked_column_chart = null;
  $scope.stacked_column_chart = new google.visualization.ColumnChart(document.getElementById('main_chart_div'));
  google.visualization.events.addListener($scope.stacked_column_chart, 'ready', function() { $timeout($scope.clear_progress, 0);});
  $scope.stacked_column_chart.draw(chart_data, options);
*/

class V1UserItem < ActiveRecord::Base
  ...
  def self.get_last_week_total(user_id, item_id, today)
    ...
    total = V1UserItem.find(:first, :select => "SUM(count) AS total", :conditions => [query, user_id, item_id, days]).total
  ...
  def self.get_data_as_hash(user_id, items, start_date, end_date)
    ...
    uitems = V1UserItem.get_user_items_and_healthscores_as_hash(user_id, items, start_date, end_date)
    ...
    start_date.upto(end_date) do |date|
      ...
      uitems.each_pair do |key, value|
        if (V1UserPreference::HEALTHSCORE == key)
          data[key] << V1UserItem.process_healthscore(value[datestr])
        elsif (Item::WEIGHT_ID == Integer(key))
          data[key] << V1UserItem.process_weight(value[datestr], data[key][-1])
        else
          data[key] << V1UserItem.process_value(value[datestr], Healthscore.smart_exists?(user_id, datestr, uitems[V1UserPreference::HEALTHSCORE]))
        end
      end
    end
  ...
  def self.get_user_items_as_hash(uid, items, start_date, end_date)
    ...
    items.each do |id|
      ...
      V1UserItem.find_all_by_user_id_and_item_id(uid, id, :conditions => V1UserItem.get_date_conditions(start_date, end_date)).each do |item|
  ...
  def self.get_user_items_and_healthscores_as_hash(uid, items, start_date, end_date)
    uitems = V1UserItem.get_user_items_as_hash(uid, items, start_date, end_date)
    uitems[V1UserPreference::HEALTHSCORE] = {}

    Healthscore.find_all_by_user_id(uid, :conditions => V1UserItem.get_date_conditions(start_date, end_date)).each do |hs|
      uitems[V1UserPreference::HEALTHSCORE][String(hs.date)] = hs
    end
